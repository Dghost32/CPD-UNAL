...
public class ReciprocalArraySumTask extends RecursiveAction {
    ...
    protected void compute() {
        final int SEQUENTIAL_THRESHOLD = 5000;

        if (endIndexExclusive - startIndexInclusive <= SEQUENTIAL_THRESHOLD) {
            IntStream.range(startIndexInclusive, endIndexExclusive).forEach(i ->
                value += 1 / input[i]
            );
        } else {
            int mid = (startIndexInclusive + endIndexExclusive) / 2;
            ReciprocalArraySumTask leftTask = new ReciprocalArraySumTask(
                startIndexInclusive,
                mid,
                input
            );
            ReciprocalArraySumTask rightTask = new ReciprocalArraySumTask(
                mid,
                endIndexExclusive,
                input
            );
