...
public class ReciprocalArraySumTest extends TestCase {
  // Número de veces que se debe repetir cada prueba para dar consistencia de los
  // resultados de en el tiempo.
  ...
  /**
   * Crea un arreglo double[] de longitud N para utilizar como entrada para las
   * pruebas.
   *
  ...
  private double[] createArray(final int N) {
    ...
    for (int i = 0; i < N; i++) {
      ...
      // No se permiten valores en cero en el arreglo de entrada para evitar la
      // división por cero
      if (input[i] == 0.0) {
        i--;
      }
  ...
  /**
   * Una implementación de referencia de seqArraysum, en caso de que alguno del
   * archvo del código fuente principal sea modificado accidentalmente.
   *
  ...
  /**
   * Una función 'helper' para hacer las pruebas de la implementación de dos y la
   * implementación de muchas tareas en paralelo.
   *
   * @param N                  Tamaño del arreglo utilizado para las pruebas
   * @param useManyTaskVersion Switch entre el código la versión de dos tareas en
   *                           paralelo y la versión de muchas tareas en paralelo
   * @param ntasks             Número de tareas a utilizar
   * @return La mejora en la rapidez (speedup) alcanzada, no todas las pruebas
  ...
  private double parTestHelper(final int N, final boolean useManyTaskVersion, final int ntasks) {
    ...
    final String errMsg = String
        .format("No concuerda el resultado para N = %d, valor esperado = %f, valor calculado = %f, error " +
            "absoluto = %f", N, correct, sum, err);
    assertTrue(errMsg, err < 1E-2);

    /*
     * Ejecuta varias repeticiones de la versiones secuncial y paralela para obtener
     * una medida más exacta del desempeño paralelo.
     */
  ...
  /**
   * Prueba que la implementación de dos tareas en paralelo calcula correctamente
   * los resultados para arreglos con un millón de elementos.
   */
  ...
  /**
   * Prueba que la implementación de dos tareas en paralelo calcula correctamente
   * los resultados para arreglos con cientos de millones de elementos..
   */
  ...
  /**
   * Prueba que la implementación de muchas tareas en paralelo calcula
   * correctamente los resultados para arreglos con un millónde elementos.
   */
  ...
  /**
   * Prueba que la implementación de muchas tareas en paralelo calcula
   * correctamente los resultados para arreglos con cientos de millones de
   * elementos.
   */
